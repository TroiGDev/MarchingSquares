...
def drawWorld():
    ...
    for x in range(len(world)-1):
        for y in range(len(world[x])-1):
            ...
            tl = 0
            if world[x][y] > threshold: tl = 1
            tr = 0
            if world[x + 1][y] > threshold: tr = 1
            bl = 0
            if world[x][y + 1] > threshold: bl = 1
            br = 0
            if world[x + 1][y + 1] > threshold: br = 1
...
while running:
    ...
    #increase threshold and loop back on max
    threshold += 0.0025
    if threshold > 1:
        threshold = 0
